...
// 响应式数据
let TypeActive = proxy.$route.query.hasOwnProperty("typeid")
    ? proxy.$route.query.typeid
    : 0;
...
// 挂载前
onBeforeMount((options) => {
    // 如果有关键词，就要把关键词赋值给搜索框
    if (proxy.$route.query.hasOwnProperty("keywords")) {
        keywords.value = proxy.$route.query.keywords;
    }
    type();
    load();
});
