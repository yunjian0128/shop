<template>
    ...
    <div class="detailbox">
        ...
        <div class="price">
            ...
            <b
                >库存：{{ product.stock }}
                {{ product.unit ? product.unit.name : "" }}</b
            >
        </div>
    </div>
    ...
    <div class="detailbox_2">
        <p class="tit">产品参数：</p>
        <div class="decribetion">{{ product.content }}</div>
    </div>
...
// 响应式数据
let proid = proxy.$route.query.hasOwnProperty("proid")
    ? proxy.$route.query.proid
    : 0;
proid = ref(proid);
