<template>
    ...
    <van-form @submit="profile">
        <van-cell-group inset>
            ...
            <van-field v-model="business.mobile" name="mobile" label="手机号" placeholder="手机号" :rules="rules.mobile" />

            <van-field v-model="business.nickname" name="nickname" label="昵称" placeholder="昵称" :rules="rules.nickname" />

            <van-field type="password" v-model="business.password" name="password" label="密码" placeholder="密码" />

            <van-field v-model="business.email" name="email" label="邮箱" placeholder="邮箱" :rules="rules.email" />
            ...
            <!-- 城市数据 -->
            <van-field v-model="business.region_text" is-link readonly name="region" label="所在地区" placeholder="所在地区"
                @click="ShowRegion = true" />

            <!-- 弹出层 -->
            <van-popup v-model:show="ShowRegion" position="bottom">
                <!-- 地区组件 -->
                <van-area v-model="RegionCode" :area-list="areaList" @confirm="RegionConfirm"
                    @cancel="ShowRegion = false" />
            </van-popup>

            <van-button round block type="primary" native-type="submit" :style="{ marginTop: '40px' }">
                提交
            </van-button>
        </van-cell-group>
    </van-form>
...
let rules = {
    ...
    email: [
        ...
        { pattern: /^\w{3,}(\.\w+)*@[A-z0-9]+(\.[A-z]{2,5}){1,2}$/, message: '邮箱格式有误' }
    ],
}
